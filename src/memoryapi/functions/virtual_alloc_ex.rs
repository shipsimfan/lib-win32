...
use crate::{
    GetLargePageMinimum, GetLastError, GetSystemInfo, LocalAlloc, VirtualAllocExNuma,
    VirtualFreeEx, VirtualProtectEx, ERROR_INVALID_ADDRESS, MEM_COMMIT, MEM_LARGE_PAGES,
    MEM_PHYSICAL, MEM_RESERVE, MEM_RESET, MEM_RESET_UNDO, MEM_TOP_DOWN, PAGE_EXECUTE,
...
unsafe extern "system" {
    ...
    ///                determines where to allocate the region. If this address is within an
    ///                enclave that you have not initialized by calling [`InitializeEnclave`],
    ///                [`VirtualAllocEx`] allocates a page of zeros for the enclave at that
    ///                address. The page must be previously uncommitted, and will not be measured
    ///                with the `EEXTEND` instruction of the Intel Software Guard Extensions
    ///                programming model. If the address in within an enclave that you initialized,
    ///                then the allocation operation fails with the [`ERROR_INVALID_ADDRESS`]
    ///                error. That is true for enclaves that do not support dynamic memory
    ///                management (i.e. SGX1). SGX2 enclaves will permit allocation, and the page
    ///                must be accepted by the enclave after it has been allocated.
    ...
    ///                       [`VirtualAllocEx`] with `MEM_COMMIT | MEM_RESERVE`. Attempting to
    ...
    ///                        [`VirtualAllocEx`] again with [`MEM_COMMIT`]. To reserve and commit
    ///                        pages in one step, call [`VirtualAllocEx`] with
    ...
    ///                      it. When you use [`MEM_RESET`], the [`VirtualAllocEx`] function
    ///                      ignores the value of `protect`. However, you must still set `protect`
    ///                      to a valid protection value, such as [`PAGE_NOACCESS`].
    ///                      [`VirtualAllocEx`] returns an error if you use [`MEM_RESET`] and the
    ...
    ///                           [`VirtualAllocEx`] function ignores the value of `protect`.
