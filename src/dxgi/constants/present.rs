...
/// Direct3D 11: This enumeration value is supported starting with Windows 8.
///
/// Note: This flag should only be used with swap effect [`DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL`] or
/// [`DXGI_SWAP_EFFECT_FLIP_DISCARD`]. The use of this flag with other swap effects is being
...
///    switching using `IDXGIFactory::make_window_association`. UWP apps that enter fullscreen mode
///    by calling `Windows::UI::ViewManagement::ApplicationView::TryEnterFullscreen()` are
///    fullscreen borderless windows and may use the flag.
///
