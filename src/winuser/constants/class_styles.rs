...
/// other windows that were obscured. Therefore, the system does not send WM_PAINT messages to
/// windows that were obscured if the memory used by the bitmap has not been discarded and if other
...
/// time required to display the window, because the system must first allocate memory to store the
/// bitmap.
