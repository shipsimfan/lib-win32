...
extern "system" {
    ...
    /// [`null_mut`], the function returns a value of zero. In both of these cases, pcbSize is set
    /// to the minimum size required for the `data` buffer.
    ///
    /// Call [`GetLastError`] to identify any other errors.
    pub fn GetRawInputDeviceInfoW(device: HANDLE, command: UINT, data: LPVOID, size: PUINT)
        -> UINT;
}
