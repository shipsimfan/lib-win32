...
/// being read by [`ReadFile`] or [`ReadConsole`], other control keys are processed by the system
/// and are not returned in the [`ReadFile`] or [`ReadConsole`] buffer. If the
/// [`ENABLE_LINE_INPUT`] mode is also enabled, backspace, carriage return, and line feed
...
/// read. If this mode is disabled, the functions return when one or more characters are available.
...
/// input buffer. These events are discarded by [`ReadFile`] or [`ReadConsole`], even when this
/// mode is enabled. The [`ReadConsoleInput`] function can be used to read [`MOUSE_EVENT`] input
...
/// [`ReadFile`] or [`ReadConsole`] function are parsed for ASCII control sequences, and the
/// correct action is performed. Backspace, tab, bell, carriage return, and line feed characters
/// are processed. It should be enabled when using control sequences or when
...
/// [`ReadConsole`], the cursor moves to the beginning of the next row when it reaches the end of
/// the current row. This causes the rows displayed in the console window to scroll up
/// automatically when the cursor advances beyond the last row in the window. It also causes the
/// contents of the console screen buffer to scroll up (../discarding the top row of the console
/// screen buffer) when the cursor advances beyond the last row in the console screen buffer. If
/// this mode is disabled, the last character in the row is overwritten with any subsequent
/// characters.
pub const ENABLE_WRAP_AT_EOL_OUTPUT: DWORD = 0x0002;

/// When writing with [`WriteFile`] or [`WriteConsole`], characters are parsed for VT100 and
/// similar control character sequences that control cursor movement, color/font mode, and other
/// operations that can also be performed via the existing Console APIs.
...
/// was off, but the next printable character will be printed as if [`ENABLE_WRAP_AT_EOL_OUTPUT`]
/// is on. No overwrite will occur. Specifically, the cursor quickly advances down to the following
...
/// the color of the foreground and background of text. Additionally, a range of DBCS flags was
/// specified with the [`COMMON_LVB`] prefix. Historically, these flags only functioned in DBCS
